...
const baseUploadDir = path.join(__dirname, 'uploads'); // Adjusted to keep all uploads in a subdirectory named 'uploads'
...
app.use(cors());
app.use('/uploads', express.static(baseUploadDir)); // Serve the uploaded images as static files
...
// Helper function to ensure directory exists with additional logging
const ensureDirectoryExistence = (dir) => {
  console.log("Checking directory existence: ", dir); // Debugging
  if (!fs.existsSync(dir)) {
    console.log("Directory does not exist. Creating: ", dir); // Debugging
    fs.mkdirSync(dir, { recursive: true });
  } else {
    console.log("Directory already exists: ", dir); // Debugging
  }
};
...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    ...
    // Define the path for each document type, based on the MTOP ID and document type
    const uploadDir = path.join(baseUploadDir, documentType, mtop_id);

    // Log the path being created for debugging
    console.log("Saving file to: ", uploadDir);

    // Ensure the directory for this specific document type and MTOP ID exists
  ...
  filename: function (req, file, cb) {
    ...
    console.log("File being saved with name: ", `${mtop_id}-${file.fieldname}-${Date.now()}${ext}`);

    // Include the mtop_id in the filename to make it easy to find the documents later
...
app.post('/upload-documents', upload.fields([
  ...
  try {
    ...
    // Debugging: Log the received fields and files
...
// Route to serve the uploaded document images
