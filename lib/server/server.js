...
app.get('/reports', async (req, res) => {
  try {
    ...
    const totalInspectionsResult = await pool.query('SELECT COUNT(*) AS count FROM inspections');
    const totalInspections = parseInt(totalInspectionsResult.rows[0].count, 10);

    // Get the most common reason for not being approved
    const commonReasonResult = await pool.query(`
      SELECT reason_not_approved, COUNT(reason_not_approved) AS count 
      FROM inspections 
      WHERE reason_not_approved IS NOT NULL 
      GROUP BY reason_not_approved 
      ORDER BY count DESC 
      LIMIT 1
    `);
    const commonReasonNotApproved = commonReasonResult.rows.length > 0
      ? commonReasonResult.rows[0].reason_not_approved
      : null;

    // Return the aggregated report data
    res.json({
      totalInspections,
      commonReasonNotApproved,
    });
  } catch (error) {
