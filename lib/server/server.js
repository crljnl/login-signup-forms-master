...
app.post('/add-inspection', async (req, res) => {
  ...
  try {
    // Ensure only the logged-in inspector can add a new registration
    if (registration_type === 'New') {
      // Fetch the logged-in inspector_id from the request body
      const loggedInInspectorId = req.body.inspector_id; // Assuming it's passed in the body

      if (!loggedInInspectorId) {
        return res.status(400).json({ message: 'Logged-in inspector ID not provided' });
      }

      // Fetch the inspector ID from the database to validate
      const inspectorResult = await pool.query('SELECT inspector_id FROM inspectors WHERE inspector_id = $1', [loggedInInspectorId]);

      if (inspectorResult.rows.length === 0 || inspectorResult.rows[0].inspector_id !== loggedInInspectorId) {
        return res.status(403).json({ message: 'Unauthorized: Only the logged-in inspector can submit new registrations' });
      }
    }

    // Proceed with inspection submission logic
    // (the rest of your insert or update logic)
  } catch (error) {
