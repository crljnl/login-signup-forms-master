...
app.post('/add-inspection', async (req, res) => {
  const {
    ...
    chassis_number, vehicle_registration, not_open_pipe, light_in_sidecar, inspection_status
  } = req.body;
  ...
  try {
    ...
    // Check if the logged-in inspector is allowed to submit (for new registrations)
    if (registration_type === 'New') {
      const loggedInInspectorId = req.body.inspector_id; // Inspector ID from request body

      if (!loggedInInspectorId) {
        return res.status(400).json({ message: 'Logged-in inspector ID not provided' });
      }

      // Validate inspector from the database
    ...
    if (existingInspection.rows.length > 0) {
      ...
      await pool.query(
        ...
        inspection_status = $14
        WHERE mtop_id = $15`,
        [
          inspector_id, applicant_name, vehicle_type, registration_type,
          side_mirror, signal_lights, taillights, motor_number, garbage_can,
          chassis_number, vehicle_registration, not_open_pipe, light_in_sidecar, inspection_status, mtop_id
        ]
      );
