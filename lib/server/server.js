...
const storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    ...
    // Set the filename: mtop_id-documentType-timestamp.extension
  ...
  async (req, res) => {
    ...
    try {
      ...
      const processFile = (file) => {
        ...
        return { binary: binaryData, filePath };
      };

      // Process each uploaded file
      const barangayClearance = processFile(files.barangay_clearance ? files.barangay_clearance[0] : null);
      const policeClearance = processFile(files.police_clearance ? files.police_clearance[0] : null);
      const sssCertificate = processFile(files.sss_certificate ? files.sss_certificate[0] : null);
      const philhealthCertificate = processFile(files.philhealth_certificate ? files.philhealth_certificate[0] : null);
      const applicantFee = processFile(files.applicant_fee ? files.applicant_fee[0] : null);
      const certificateOfRegistration = processFile(files.certificate_of_registration ? files.certificate_of_registration[0] : null);
      const driversLicense = processFile(files.drivers_license ? files.drivers_license[0] : null);
      ...
      res
        .status(500)
        .json({ message: 'Failed to upload documents', error: error.message });
    }
  }
);
...
app.post('/add-inspection', async (req, res) => {
  const {
    ...
    chassis_number, vehicle_registration, not_open_pipe, light_in_sidecar, 
    inspection_status, reason_not_approved, town
  } = req.body;

  try {
    // Validate required fields
    if (!mtop_id || mtop_id.length !== 6) {
      return res.status(400).json({ message: 'MTOP ID must be exactly 6 characters' });
    }
    if (!inspector_id || !applicant_name || !vehicle_type || !town) {
    ...
    if (existingInspection.rows.length > 0) {
      ...
      await pool.query(
        ...
        SET inspector_id = $1, applicant_name = $2, vehicle_type = $3, registration_type = $4, 
            ...
            inspection_status = $14, reason_not_approved = $15, town = $16
        WHERE mtop_id = $17`,
        [
          inspector_id, applicant_name, vehicle_type, registration_type,
          side_mirror, signal_lights, taillights, motor_number, garbage_can,
          chassis_number, vehicle_registration, not_open_pipe, light_in_sidecar, 
          inspection_status, reason_not_approved, town, mtop_id
        ]
      );
